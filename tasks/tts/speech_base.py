...
class SpeechBaseTask(BaseTask):
    ...
    def test_step(self, sample, batch_idx):
        ...
        self.saving_result_pool.add_job(
            self.save_result, args=[wav_pred, mel_pred, base_fn % "P", gen_dir, str_phs]
        )
        if hparams["save_gt"]:
            wav_gt = self.vocoder.spec2wav(mel_gt)
            self.saving_result_pool.add_job(
                self.save_result, args=[wav_gt, mel_gt, base_fn % "G", gen_dir, str_phs]
            )
    ...
    @staticmethod
    def save_result(
        wav_out, mel, base_fn, gen_dir, str_phs=None, mel2ph=None, alignment=None
    ):
        save_wav(
            wav_out,
