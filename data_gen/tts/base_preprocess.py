...
class BasePreprocessor:
    ...
    @classmethod
    def build_mfa_inputs(
        cls, item, mfa_input_dir, mfa_group, wav_processed_tmp, preprocess_args
    ):
        ...
        move_link_func = (
            move_file
            if os.path.dirname(wav_align_fn) == wav_processed_tmp
            else link_file
        )
        ...
        ph_gb_word_nosil = " ".join(
            [
                ...
                for w in ph_gb_word.split(" ")
                if not is_sil_phoneme(w)
            ]
        )
