...
FS_ENCODERS = {
    ...
    "conv": lambda hp, dict_size: TextConvEncoder(
        dict_size,
        hp["hidden_size"],
        hp["hidden_size"],
        hp["enc_dilations"],
...
FS_DECODERS = {
    ...
    "rnn": lambda hp: DecoderRNN(
        hp["hidden_size"], hp["decoder_rnn_dim"], hp["dropout"]
    ),
    "conv": lambda hp: ConvBlocks(
        hp["hidden_size"],
        hp["hidden_size"],
        hp["dec_dilations"],
        hp["dec_kernel_size"],
...
class FastSpeech(nn.Module):
    def __init__(self, dict_size, hparams, out_dims=None):
        ...
        if hparams["dec_inp_add_noise"]:
            ...
            self.dec_inp_noise_proj = nn.Linear(
                self.hidden_size + self.z_channels, self.hidden_size
            )
    ...
    def forward(
        ...
        **kwargs
        ...
        style_embed = self.forward_style_embed(emotion_id, spk_embed, spk_id)

        # add dur
        dur_inp = (encoder_out + style_embed) * src_nonpadding
        # print(dur_inp.shape, mel2ph)
        ...
        if self.hparams["use_pitch_embed"]:
            ...
            decoder_inp = decoder_inp + self.forward_pitch(
                pitch_inp, f0, uv, mel2ph, ret, encoder_out
            )
        ...
        if self.hparams["dec_inp_add_noise"]:
            ...
            z = kwargs.get("adv_z", torch.randn([B, T, self.z_channels])).to(
                decoder_inp.device
            )
            ret["adv_z"] = z
    ...
    def forward_style_embed(self, emotion_id=None, spk_embed=None, spk_id=None):
        # add spk embed
        style_embed = 0
        ...
        style_embed = style_embed + self.emotion_id_proj(emotion_id)[:, None, :]
    ...
    def forward_dur(self, dur_input, mel2ph, txt_tokens, ret):
        ...
        if self.hparams["predictor_grad"] != 1:
            dur_input = dur_input.detach() + self.hparams["predictor_grad"] * (
    ...
    def forward_pitch(self, decoder_inp, f0, uv, mel2ph, ret, encoder_out=None):
        ...
        if self.hparams["pitch_type"] == "ph":
            ...
            ret["f0_denorm_pred"] = torch.gather(
                F.pad(ret["f0_denorm_pred"], [1, 0]), 1, mel2ph
            )
